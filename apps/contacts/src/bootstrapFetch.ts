// apps/contacts/src/bootstrapFetch.ts
...
function storeOrgId(orgId: number) {
  ...
  if (Number(orgId) > 0) {
    ...
    try { localStorage.setItem("BHQ_ORG_ID", String(orgId)); } catch { }
  }
}
...
async function devLogin(orgId: number): Promise<void> {
  if (isProd) return; // do nothing in prod

  // Build /auth/dev-login (same-origin). Cookie is first-party thanks to Vite proxy.
  let url = `/api/v1/auth/dev-login?email=dev@local&orgId=${encodeURIComponent(orgId)}`;
  const useQueryToken = true; // easiest; server accepts ?token=...
  if (DEV_TOKEN && useQueryToken) url += `&token=${encodeURIComponent(DEV_TOKEN)}`;

  const res = await fetch(url, {
    credentials: "include",
    // If you prefer header auth for dev-login, uncomment next line and remove ?token= above:
    // headers: DEV_TOKEN ? { Authorization: `Bearer ${DEV_TOKEN}` } : undefined,
  });
  if (!res.ok) {
    const t = await res.text().catch(() => "");
...
export async function bootstrapAuthAndOrg(): Promise<void> {
  ...
  if (!me && !isProd) {
    if (orgId == null) {
      // Last chance: if we STILL don't know an org, throw a clear error so it’s obvious.
      throw new Error(
  ...
  if (orgId == null) {
    // Minimal safe fallback: do nothing (your request layer will still 400 with org_required).
    // Or throw to force you to build the org picker now:
    // throw new Error("Multiple organizations. Implement org picker to set window.__BHQ_ORG_ID__ / localStorage.BHQ_ORG_ID");
  } else {
    storeOrgId(orgId);
  }
}

// Optionally run automatically on import (comment out if you call it yourself in main.tsx)
// (async () => { await bootstrapAuthAndOrg(); })();
