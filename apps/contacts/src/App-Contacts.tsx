...
/* ────────────────────────────────────────────────────────────────────────────
 * NEW: make portal hosts clickable only when populated
 * This watches each host and flips pointer-events to 'auto' while it has children.
 * (When empty, it reverts to 'none' so it won't block the app.)
 * ────────────────────────────────────────────────────────────────────────── */
...
const ContactDetailsView: React.FC<ContactDetailsViewProps> = ({
  ...
  const phonePortal = getGlobalHost("bhq-flyout-root");
...
export default function AppContacts() {
  const phonePortal = React.useMemo(() => getGlobalHost("bhq-flyout-root"), []);
