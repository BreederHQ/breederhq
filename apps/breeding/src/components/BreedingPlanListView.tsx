// apps/breeding/src/components/BreedingPlanListView.tsx
// List view for breeding plans - cleaner row-based UI
...
type BreedingPlanListViewProps = {
  rows: PlanRow[];
  loading: boolean;
  error: string | null;
...
const STATUS_COLORS: Record<string, string> = {
  ...
  WEANED: "hsl(80, 60%, 45%)",
  PLACEMENT_STARTED: "hsl(142, 70%, 45%)",
  PLACEMENT_COMPLETED: "hsl(160, 60%, 42%)",
...
const COLUMN_WIDTHS_PX: Record<string, number> = {
  ...
  code: 96,
  tags: 140,
  expectedCycleStart: 96,
  expectedHormoneTestingStart: 96,
  expectedBreedDate: 96,
  expectedBirthDate: 96,
  expectedWeaned: 96,
  expectedPlacementStartDate: 96,
  expectedPlacementCompletedDate: 96,
  cycleStartDateActual: 96,
  hormoneTestingStartDateActual: 96,
  breedDateActual: 96,
  birthDateActual: 96,
  weanedDateActual: 96,
  placementStartDateActual: 96,
  placementCompletedDateActual: 96,
  completedDateActual: 96,
  lockedCycleStart: 96,
  lockedOvulationDate: 96,
  lockedDueDate: 96,
  lockedPlacementStartDate: 96,
};
...
function CellValue({ row, colKey }: { row: PlanRow; colKey: string }) {
...
export function BreedingPlanListView({
  rows,
  loading,
  error,
  onRowClick,
  visibleColumns,
}: BreedingPlanListViewProps) {
  if (loading) {
    return (
