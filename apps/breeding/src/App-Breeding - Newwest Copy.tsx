...
function WhatIfRowEditor(props: WhatIfRowEditorProps) {
  const { row, females, api, onChange, onConvertToPlan, onRemove } = props;
  ...
  const asISODateOnly = (v: any): string | null => {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        try {
          data = await (api as any).getAnimal?.(row.damId, include);
        } catch {
          data = null;
        }

        // Fallback when getAnimal is missing OR returns null/undefined without throwing
        if (!data) {
          const res = await (api as any).get?.(`/animals/${row.damId}`, {
            params: { include },
          });

          // tolerate multiple response shapes (axios, fetch wrappers, etc.)
          data =
        ...
        console.warn("[BHQ reproEngine][whatif raw animal payload]", {
          ...
          dataKeys: (data as any)?.data ? Object.keys((data as any).data) : null,
          animalKeys: (data as any)?.animal ? Object.keys((data as any).animal) : null,
        ...
        if (!last_heat && cycleStartDates.length) {
          last_heat = cycleStartDates[cycleStartDates.length - 1] ?? null;
        }
  ...
  const projectedCycles = React.useMemo(() => {
    if (!row.damId) return [];
    if (!cycleStartsAsc.length) return [];

    const todayIso = new Date().toISOString().slice(0, 10);

    const result = projectUpcomingCycleStarts(
      {
        animalId: String(row.damId),
        species: (row.species ?? "DOG") as any,
        dob: null,
        cycleStartsAsc,
        today: todayIso,
        lastBredAt: null,
      ...
      { horizonMonths: 18, maxCount: 8 }
    );

    return result.projected.map((p) => p.date);
  }, [row.damId, row.species, cycleStartsAsc]);
  ...
  return (
    <div className="rounded-lg border border-hairline bg-surface p-2">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-[220px]">
          <div className="mb-1 text-xs text-secondary">Cycle start (projected)</div>
          <select
            className="h-8 w-full rounded-md border border-hairline bg-surface-subtle px-2 text-sm text-primary"
...
function PlanDetailsView(props: {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        try {
          const res: any = await (api as any).getAnimal?.(
            row.damId,
            "repro,last_heat,lastCycle,cycleStartDates"
          );

          if (res) {
            const reproRaw: DamReproEvent[] = Array.isArray(res?.repro)
              ? (res.repro as DamReproEvent[])
              : [];

            const repro: DamReproEvent[] = reproRaw
              .filter((e: any) => e && e.date)
              .map((e: any) => {
                const kind =
                  e.kind === "cycle_start"
                    ? ("heat_start" as DamReproEvent["kind"])
                    : (e.kind as DamReproEvent["kind"]);

                const d = asISODateOnlyEngine(e.date);
                return d ? ({ ...e, kind, date: d } as DamReproEvent) : null;
              })
              .filter(Boolean) as DamReproEvent[];

            const cycleStartDates: string[] = Array.isArray(res?.cycleStartDates)
              ? (res.cycleStartDates as any[])
                .map((d) => asISODateOnlyEngine(d))
                .filter(Boolean)
                .sort()
              : [];

            const lastCycle: string | null = asISODateOnlyEngine(
              res?.lastCycle ?? res?.last_cycle ?? null
            );

            let last_heat: string | null = asISODateOnlyEngine(
              res?.last_heat ?? res?.lastHeat ?? null
            );

            if (!last_heat && repro.length) {
              const heats = repro
                .filter((e) => e.kind === "heat_start" && e.date)
                .map((e) => asISODateOnlyEngine(e.date))
                .filter(Boolean)
                .sort();
        ...
        if (!cancelled) {
          setDamLoadError(e?.message || "Failed to load Dam cycle history");
        }
      }
    })();
  ...
  /*
    BHQ Step 9 LEGACY plan drawer cycle seed (REMOVE AFTER GREENFIELD CUTOVER)
    ...
    const lastHeatStart =
      asISODateOnly(damRepro?.last_heat) ??
      (damHeatStartsAsc.length ? damHeatStartsAsc[damHeatStartsAsc.length - 1] : null);
  ...
  */
  ...
  // Step 9 greenfield seed: canonical cycleStartsAsc from all available sources
  const cycleStartsAsc = React.useMemo(() => {
    const dates: string[] = [];
  ...
  // Step 9 greenfield projection list (this is what the dropdown must use)
  const projectedCycles = React.useMemo(() => {
    if (!row.damId) return [];
    if (!cycleStartsAsc.length) return [];
    ...
    React.useEffect(() => {
      if (!row.damId) return;
      console.warn("[BHQ reproEngine][whatif]", {
        damId: row.damId,
        species: row.species,
        damReproKeys: damRepro ? Object.keys(damRepro as any) : null,
        cycleStartsAsc,
        projectedCycles,
      });
    }, [row.damId, row.species, damRepro, cycleStartsAsc, projectedCycles]);
