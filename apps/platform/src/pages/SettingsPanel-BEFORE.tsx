...
const BreedingTab = React.forwardRef<BreedingHandle, { dirty: boolean; onDirty: (v: boolean) => void }>(
  function BreedingTabImpl({ onDirty }, ref) {
    ...
    /** ───────── Derived source + one-click reset for phase fields ───────── */
    // Treat platform defaults as the "derived" source shown on the right side
    const DER: AvailabilityPrefs = DEFAULTS;

    // Reset ONLY the Timeline Phases offsets to the platform defaults
    function ResetPhasesToDerived() {
      const next: any = { ...form };
      ([
      ...
      ] as (keyof AvailabilityPrefs)[]).forEach((k) => {
        next[k] = (DER as any)[k] ?? 0;
      });
      setForm(next);
    }
    ...
    React.useEffect(() => {
      ...
      (async () => {
        try {
          ...
          // Load program profile with 404 tolerance (via api client)
          ...
          try {
            ...
            if (e?.status === 404) {
              ...
              // keep UI alive but surface error
              profMerged = { ...EMPTY_PROFILE };
              setError(e?.message || "Program profile load failed");
            }
          }
    ...
    async function saveAll() {
      ...
      try {
        ...
        // Save program profile if changed (PUT)
        ...
        {loading ? (
          ...
          <div className="space-y-5">
            <div className="rounded-md border border-hairline p-3 bg-surface">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium">Testing → Breeding</div>
                  <Hint>Unlikely wraps the likely span. Risky wraps the full span.</Hint>
                </div>
                <span className="text-[10px] px-1.5 py-[1px] rounded border border-hairline text-tertiary">
                  Platform default → Tenant override
                </span>
              </div>
              <div className="mt-3 space-y-3">
                <TwoCol>
                  <FieldNumDerived
                    label="Unlikely: days before"
                    value={Number(form.testing_unlikely_from_likely_start || 0)}
                    derived={Number((DER as any).testing_unlikely_from_likely_start || 0)}
                    onChange={(n) => _setPref("testing_unlikely_from_likely_start", n)}
                    onReset={() => _setPref("testing_unlikely_from_likely_start", Number((DER as any).testing_unlikely_from_likely_start || 0))}
                    invalid={phaseInvalid(Number(form.testing_unlikely_from_likely_start || 0), Number(form.testing_unlikely_to_likely_end || 0))}
                  />
                  <FieldNumDerived
                    label="Unlikely: days after"
                    value={Number(form.testing_unlikely_to_likely_end || 0)}
                    derived={Number((DER as any).testing_unlikely_to_likely_end || 0)}
                    onChange={(n) => _setPref("testing_unlikely_to_likely_end", n)}
                    onReset={() => _setPref("testing_unlikely_to_likely_end", Number((DER as any).testing_unlikely_to_likely_end || 0))}
                    invalid={phaseInvalid(Number(form.testing_unlikely_from_likely_start || 0), Number(form.testing_unlikely_to_likely_end || 0))}
                  />
                </TwoCol>
                <TwoCol>
                  <FieldNumDerived
                    label="Risky: days before"
                    value={Number(form.testing_risky_from_full_start || 0)}
                    derived={Number((DER as any).testing_risky_from_full_start || 0)}
                    onChange={(n) => _setPref("testing_risky_from_full_start", n)}
                    onReset={() => _setPref("testing_risky_from_full_start", Number((DER as any).testing_risky_from_full_start || 0))}
                  />
                  <FieldNumDerived
                    label="Risky: days after"
                    value={Number(form.testing_risky_to_full_end || 0)}
                    derived={Number((DER as any).testing_risky_to_full_end || 0)}
                    onChange={(n) => _setPref("testing_risky_to_full_end", n)}
                    onReset={() => _setPref("testing_risky_to_full_end", Number((DER as any).testing_risky_to_full_end || 0))}
                  />
                </TwoCol>
              </div>
            </div>
            ...
            <div className="rounded-md border border-hairline p-3 bg-surface">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium">Birth → Placement</div>
                  <Hint>Unlikely wraps the likely span. Risky wraps the full span.</Hint>
                </div>
                <span className="text-[10px] px-1.5 py-[1px] rounded border border-hairline text-tertiary">
                  Platform default → Tenant override
              ...
              <div className="mt-3 space-y-3">
                <TwoCol>
                  <FieldNumDerived
                    label="Unlikely: days before"
                    value={Number(form.post_unlikely_from_likely_start || 0)}
                    derived={Number((DER as any).post_unlikely_from_likely_start || 0)}
                    onChange={(n) => _setPref("post_unlikely_from_likely_start", n)}
                    onReset={() => _setPref("post_unlikely_from_likely_start", Number((DER as any).post_unlikely_from_likely_start || 0))}
                    invalid={phaseInvalid(Number(form.post_unlikely_from_likely_start || 0), Number(form.post_unlikely_to_likely_end || 0))}
                  />
                  <FieldNumDerived
                    label="Unlikely: days after"
                    value={Number(form.post_unlikely_to_likely_end || 0)}
                    derived={Number((DER as any).post_unlikely_to_likely_end || 0)}
                    onChange={(n) => _setPref("post_unlikely_to_likely_end", n)}
                    onReset={() => _setPref("post_unlikely_to_likely_end", Number((DER as any).post_unlikely_to_likely_end || 0))}
                    invalid={phaseInvalid(Number(form.post_unlikely_from_likely_start || 0), Number(form.post_unlikely_to_likely_end || 0))}
                  />
                </TwoCol>
                <TwoCol>
                  <FieldNumDerived
                    label="Risky: days before"
                    value={Number(form.post_risky_from_full_start || 0)}
                    derived={Number((DER as any).post_risky_from_full_start || 0)}
                    onChange={(n) => _setPref("post_risky_from_full_start", n)}
                    onReset={() => _setPref("post_risky_from_full_start", Number((DER as any).post_risky_from_full_start || 0))}
                  />
                  <FieldNumDerived
                    label="Risky: days after"
                    value={Number(form.post_risky_to_full_end || 0)}
                    derived={Number((DER as any).post_risky_to_full_end || 0)}
                    onChange={(n) => _setPref("post_risky_to_full_end", n)}
                    onReset={() => _setPref("post_risky_to_full_end", Number((DER as any).post_risky_to_full_end || 0))}
                  />
                </TwoCol>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <Button size="sm" variant="outline" onClick={ResetPhasesToDerived}>Reset phases to platform defaults</Button>
              <Hint>Resets only the phase fields on this page.</Hint>
            </div>
          </div>
        )}
      </SectionCard>
    ...
    const DatesTab = (
      <SectionCard
        ...
        right={<span className="text-[10px] px-1.5 py-[1px] rounded border border-hairline text-tertiary">
          Platform default → Tenant override
        </span>}
      >
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            {/* Placement Start enable toggle remains unchanged */}
...
/** ───────── Soft validation for bands ───────── */
function bandWidth(from: number, to: number) { return (to || 0) - (from || 0); }

function exactDateInvalid(from: number, to: number): boolean {
  // For a single anchor, a negative width means the window collapses/inverts.
  return bandWidth(from, to) < 0;
}

function phaseInvalid(beforeDays: number, afterDays: number): boolean {
  // For phase wraps, negative values are allowed, but a negative combined width is non-sensical.
...
const HELP_BOX = (
  <Card className="p-3">
    ...
    <p className="text-xs text-tertiary">
      ...
      calendar date. Risky represents a strong degree of potential avaibility conflict. Unlikely indicates periods of soft caution windows.
    </p>
  </Card>
);

/** ───────── Mini preview: shows how offsets render ───────── */
function MiniPreview({
  form,
  placementEnabled,
}: { form: AvailabilityPrefs; placementEnabled: boolean }) {
  // Build a few mock anchors (today + offsets)
  const today = new Date();
  function d(n: number) { const x = new Date(today); x.setDate(x.getDate() + n); return x; }
  const anchors = [
    {
      label: "Cycle", f: form.date_cycle_risky_from, t: form.date_cycle_risky_to,
      uf: form.date_cycle_unlikely_from, ut: form.date_cycle_unlikely_to
    },
    {
      label: "Breed", f: form.date_breeding_risky_from, t: form.date_breeding_risky_to,
      uf: form.date_breeding_unlikely_from, ut: form.date_breeding_unlikely_to
    },
    {
      label: "Birth", f: form.date_birth_risky_from, t: form.date_birth_risky_to,
      uf: form.date_birth_unlikely_from, ut: form.date_birth_unlikely_to
    },
    ...(placementEnabled ? [{
  ...
  // simple linear map: day offset → x position
  const W = 320, H = 12, PAD = 6, TOTAL = 120; // show 120-day window
          ...
          return (
            <div key={a.label} className="flex items-center gap-2">
              ...
              <svg width={W} height={H + 2} className="shrink-0">
                ...
                {/* Unlikely */}
                <rect x={cx(a.uf)} y={1} width={unlikW} height={H - 2}
                  fill="currentColor" opacity="0.15" stroke="currentColor" strokeOpacity="0.4" />
                {/* Risky */}
                <rect x={cx(a.f)} y={3} width={riskyW} height={H - 6}
                  fill="currentColor" opacity="0.3" stroke="currentColor" strokeOpacity="0.5" />
              </svg>
            </div>
          );
        })}
...
/** ───────── Derived vs Override helpers ───────── */
function DeltaChip({ delta }: { delta: number }) {
  if (!Number.isFinite(delta) || delta === 0) return null;
  const sign = delta > 0 ? "+" : "";
  return (
    <span className="ml-2 inline-flex items-center rounded px-1.5 py-[1px] text-[10px] border
      border-[hsl(var(--brand-orange))/40] text-[hsl(var(--brand-orange))] bg-[hsl(var(--brand-orange))/10]">
      {sign}{delta}
    </span>
  );
}
  ...
  return (
    <label className="space-y-1">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-tertiary">Derived:</span>
        <span className="text-[11px]">{Number.isFinite(derived) ? derived : 0}</span>
        <DeltaChip delta={delta} />
        {onReset && (
          <button
            type="button"
            className="ml-2 text-[11px] px-2 py-0.5 rounded border border-hairline text-secondary hover:text-primary"
            onClick={onReset}
      ...
      <input
        ...
        className={[
          ...
          invalid ? "border-red-500/60 focus:shadow-[0_0_0_2px_rgba(244,63,94,.6)]"
            : "border-hairline"
        ].join(" ")}
...
function ExactDateRow(props: {
  ...
  derivedDefaults?: AvailabilityPrefs; // NEW (optional); falls back to DEFAULTS
}) {
  const {
    title, riskyFromKey, riskyToKey, unlikelyFromKey, unlikelyToKey,
    form, setForm, derivedDefaults
  } = props;

  const DER = derivedDefaults || DEFAULT_AVAILABILITY_PREFS;

  const riskyFrom = Number(form[riskyFromKey] ?? 0);
  const riskyTo = Number(form[riskyToKey] ?? 0);
  const unrFrom = Number(form[unlikelyFromKey] ?? 0);
  const unrTo = Number(form[unlikelyToKey] ?? 0);

  const invalidRisky = exactDateInvalid(riskyFrom, riskyTo);
  const invalidUnlk = exactDateInvalid(unrFrom, unrTo);
  ...
  return (
    <div className="rounded-md border border-hairline p-3 bg-surface space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] px-1.5 py-[1px] rounded border border-hairline text-tertiary">
            <span className="opacity-60">Provenance:</span> Platform default → Tenant override
          </span>
          <Button size="xs" variant="outline" onClick={resetRow} title="Reset all four fields in this row">
            Reset row
          </Button>
        </div>
      </div>
