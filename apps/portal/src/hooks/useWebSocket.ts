// apps/portal/src/hooks/useWebSocket.ts
// WebSocket hook for real-time messaging in client portal
...
export type WebSocketEvent =
  ...
  | { event: "new_email"; payload: { id: string; type: "partyEmail" | "unlinkedEmail"; toEmail: string; subject: string; preview: string; sentAt: string } }
  | { event: "session_invalidated"; payload: { reason: "suspended" | "password_reset" | "logged_out" } };
...
export function useWebSocket({
  ...
  const createConnection = useCallback(() => {
    if (!mountedRef.current || !enabledRef.current || !tenantIdRef.current) {
      return;
    }
    ...
    console.log("[WS Portal] Connecting to:", wsUrl);
    ...
    try {
      ...
      console.error("[WS Portal] Failed to create WebSocket:", err);
      return;
    }
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        console.log("[WS Portal] Received:", data.event);
        onMessageRef.current?.(data);
      } catch (err) {
        console.error("[WS Portal] Failed to parse message:", err);
      }
    };

    ws.onclose = (event) => {
      if (!mountedRef.current) return;
      console.log("[WS Portal] Disconnected:", event.code, event.reason);
      ...
      if (enabledRef.current && reconnectAttemptsRef.current < 10 && mountedRef.current) {
        ...
        console.log(`[WS Portal] Reconnecting in ${delay}ms (attempt ${reconnectAttemptsRef.current + 1})`);
    ...
    ws.onerror = (error) => {
      console.error("[WS Portal] Error:", error);
    };
  }, []);
