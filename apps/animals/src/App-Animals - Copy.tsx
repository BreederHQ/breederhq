...
function CycleTab({
  ...
  const add = (v: string) => {
    if (!v) return;
    setDates((prev) => Array.from(new Set([...prev, v])).sort());
  };

  const remove = (d: string) => {
    if (!window.confirm("Delete this cycle start date?")) return;
...
const persist = React.useCallback(
  async (next: string[]) => {
    ...
    setWorking(true);
    try {
      await api?.animals?.putCycleStartDates({ animalId: id, dates: next });
      toast.success("Cycle start dates saved.");
      onSaved(next);
    } catch (err) {
      console.error("[Animals] putCycleStartDates failed", err);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
          ...
          <div>
            <div className="text-xs text-secondary">
              Next Projected Cycle Start
            </div>
            <div>
              {projected.length
                ? projected.map(pretty).join(" • ")
                : ""}
            </div>
          </div>
        </div>
                ...
                {preview.full.map((e) => (
                  <li key={e.id}>
                    <span className="text-primary">
                      {e.title.replace(" (Full)", "")}:
                    </span>{" "}
                    {pretty(e.start.toISOString().slice(0, 10))} →
                    {" "}
                    {pretty(
                      new Date(e.end.getTime() - 86400000)
                        .toISOString()
                        .slice(0, 10)
                    )}
                  </li>
...
export default function AppAnimals() {
  ...
  const syncOwners = React.useCallback(
    async (animalId: number, desired: OwnershipRow[] | undefined | null) => {
      ...
      // If nothing desired, delete all existing owners
      if (!rows.length) {
        try {
      ...
      for (const d of desiredNorm) {
        ...
        if (!e) {
          ...
          // Possibly update percent or isPrimary
  ...
  // Wrapper for OwnershipEditor lookups so we always provide a display_name and label
  const ownershipLookups = React.useMemo(
    () => ({
      async searchContacts(q: string) {
        const raw = (await api?.lookups?.searchContacts?.(q)) ?? [];
        const arr =
          Array.isArray(raw) ? raw : (raw as any)?.rows ?? (raw as any)?.items ?? [];

        return (arr as any[]).map((c) => {
          const nameFromNames = [c.first_name, c.last_name].filter(Boolean).join(" ");
          ...
          return {
            ...c,
            // cover every field OwnershipEditor might try to read
            display_name: name,
            label: name,
            name,
            title: name,
      ...
      },
      {
        ...
        fields: [
          { label: "Owner", view: (r) => r.ownerName ?? "—" },
          {
            ...
            view: (r) => (
              <OwnershipChips
                owners={(((r as any).owners || []) as OwnershipRow[]).filter(
                  (o) => !o.is_primary
                )}
              />
            ),
    ...
    () => ({
      ...
      fetchRow: async (id: number) => {
        ...
        try {
          ...
          owners = items.map((o: any): OwnershipRow => ({
            partyType: o.partyType === "Organization" ? "Organization" : "Contact",
      ...
      onSave: async (id: number, draft: Partial<AnimalRow>) => {
        ...
        if (owners && owners.length) {
          ...
          ownerNameOverride = primary?.display_name ?? owners[0]?.display_name ?? undefined;
          try {
              ...
              {mode === "view" ? (
                <SectionCard title="Ownership">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {mode === "view" ? (
                      <>
                        {/* Primary Owner = only those flagged primary, or first owner as fallback */}
                        <LV label="Primary Owner">
                          <OwnershipChips
                            owners={(
                              (((row as any).owners) ?? []) as any[]
                            ).filter((o: any, idx: number, arr: any[]) => {
                              const isPrimary = o.is_primary || o.primary;
                              // If nothing is flagged primary, treat the first as primary
                              if (arr.some((x: any) => x.is_primary || x.primary)) {
                                return !!isPrimary;
                              }
                              return idx === 0;
                            })}
                        ...
                        {/* Additional Owners = everyone who is not primary */}
                        <LV label="Additional Owners">
                          <OwnershipChips
                            owners={(
                              (((row as any).owners) ?? []) as any[]
                            ).filter((o: any, idx: number, arr: any[]) => {
                              const anyPrimary = arr.some((x: any) => x.is_primary || x.primary);
                              const isPrimary = o.is_primary || o.primary || (!anyPrimary && idx === 0);
                              return !isPrimary;
                            })}
                          />
                        </LV>
                      ...
                      <>
                        ...
                        <LV label="Additional Owners">
                          <OwnershipEditor
                            ...
                            onChange={(owners) => {
                              const primary = owners.find((o: any) => o.is_primary) ?? owners[0] ?? null;
                              ...
                              const primaryName =
                                (primary &&
                                  (primary.display_name ||
                                    primary.name ||
                                    primary.party_name ||
                                    (primary.contact && primary.contact.display_name))) ||
                                null;
  ...
  const doCreateAnimal = async () => {
    ...
    try {
      ...
      if (toSaveOwners.length) {
        const primary = toSaveOwners.find((o) => o.is_primary) ?? toSaveOwners[0];
