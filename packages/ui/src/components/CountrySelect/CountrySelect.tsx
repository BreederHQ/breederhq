...
function normalizeToCode(value?: string | null, list?: { code: string; name: string }[]) {
...
function displayName(value?: string | null, list?: { code: string; name: string }[]) {
...
export function CountrySelect({
  value, onChange, countries: external, placeholder = "Country", className
}: CountrySelectProps) {
  const internal = useCountries();
  const items = external?.length ? external : internal;
