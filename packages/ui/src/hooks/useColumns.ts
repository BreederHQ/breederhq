// useColumns.ts
export function useColumns<K extends string>(defs: Array<{ key: K; default?: boolean }>, storageKey: string) {
  const defaults = React.useMemo(
    () => defs.reduce<Record<K, boolean>>((acc, d) => ({ ...acc, [d.key]: !!d.default }), {} as any),
    [defs]
  );
  ...
  React.useEffect(() => { try { localStorage.setItem(storageKey, JSON.stringify(map)); } catch {} }, [map, storageKey]);
