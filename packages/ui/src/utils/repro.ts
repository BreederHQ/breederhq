// packages/ui/src/utils/repro.ts
export type ISODate = string;
export type BioSpecies = "DOG" | "CAT" | "HORSE" | "OTHER";

export interface CycleOptions { lastN?: number; fallbackAvgDays?: number; species?: BioSpecies; rounding?: "nearest" | "floor" | "ceil"; }
export interface CycleSummary { last: ISODate | null; avgAll: number | null; avgLastN: number | null; next: ISODate | null; }

export const SPECIES_DEFAULTS: Record<BioSpecies, number> = {
  DOG: 180,
  CAT: 21,
  HORSE: 21,
  OTHER: 180,
...
function toUTCDateOnly(iso: ISODate): ISODate {
  const d = new Date(iso);
  if (Number.isNaN(d.valueOf())) return "" as ISODate;
...
function mean(arr: number[], rounding: NonNullable<CycleOptions["rounding"]>): number | null {
  ...
  const raw = arr.reduce((s, n) => s + n, 0) / arr.length;
...
export function computeCycleSummary(inputDates: ISODate[], opts: CycleOptions = {}): CycleSummary {
  ...
  const deltas: number[] = [];
  for (let i = 1; i < dates.length; i++) deltas.push(daysBetween(dates[i - 1] as ISODate, dates[i] as ISODate));
  const avgAll = mean(deltas, rounding);
  const avgLastN = mean(deltas.slice(-lastN), rounding);
  const fallback = typeof fallbackAvgDays === "number" && fallbackAvgDays > 0 ? fallbackAvgDays : SPECIES_DEFAULTS[species];
