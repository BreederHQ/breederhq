...
function resolveTenantIdFromAnySource(): number | null {
  ...
  // cookie wonΓÇÖt be readable when HttpOnly; keep the code but donΓÇÖt rely on it
...
const ALL_COLUMNS: ColumnDef[] = [
  ...
  { key: "phone", label: "Phone", default: true, type: "text", render: (r: ContactRow) => r.phone ? <span className="whitespace-nowrap">   {formatPhone(String(r.phone), countryCodeFor(r.country))} </span> : <span className="text-secondary">ΓÇö</span> },
  ...
  { key: "whatsappPhone", label: "WhatsApp", default: false, type: "text", render: (r: ContactRow) => r.whatsappPhone ? <span className="whitespace-nowrap">{formatPhone(String(r.whatsappPhone), countryCodeFor(r.country))}</span> : <span className="text-secondary">ΓÇö</span> }, { key: "city", label: "City", default: false, type: "text" },
...
const parseE164 = (v?: string) => {
  ...
  // If someone typed ΓÇ£US ΓÇªΓÇ¥ etc., map alpha-2 prefix to dial
    ...
    <>
      ...
      >
        <div className="p-2 border-b border-hairline">
          <input
            autoFocus
            placeholder="Search country or codeΓÇª"
            value={query}
...
function normalize(dto: ContactDTO): ContactRow {
  ...
  // ≡ƒæç bring top-level WhatsApp fields into commPrefs (keeps UI consistent)
...
// ΓöÇΓöÇ module-scope helpers (PUT THESE AT TOP OF FILE) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
export default function AppContacts() {
  ...
  // ΓöÇΓöÇ More menu positioning ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [tenantReady, api]); // v1 not used here, so donΓÇÖt include it
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // silently ignore; UI already shows ΓÇ£No tagsΓÇ¥ if empty
      }
    })();
...
React.useLayoutEffect(() => {
  ...
  async function bulkApplyTags() {
    ...
    try {
      ...
      // 2) Parse adds/removes by name ΓåÆ id (ignore unknowns)
  ...
  /** ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ UI ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  return (
    <>
            ...
            <div className="flex items-center gap-3 justify-between min-w-0">
              ...
              <div className="pr-2 flex-none w-full sm:w-[480px] md:w-[560px] lg:w-[640px] max-w-full">
                <div className="relative w-full">
                  ...
                  >
                    {/* ΓÇ£slidersΓÇ¥ icon that matches the screenshot style */}
              ...
              <div className="shrink-0 flex items-center gap-2">
                ...
                {/* More (Export/Import) ΓÇö portaled so it never clips */}
      ...
      <span ref={moreAnchorRef} className="inline-flex">
        <Button variant="outline" size="icon" aria-label="More" onClick={() => setMoreOpen(v => !v)} className="h-9 w-9">Γï»</Button>
      </span>
  */}
                  ...
                  {moreOpen && morePos && createPortal(
                    <>
                      ...
                      >
                        ...
                        <button
                          className="w-full text-left px-2 py-1.5 rounded hover:bg-[hsl(var(--brand-orange))]/12"
                          onClick={() => { setMoreOpen(false); alert("Import CSV ΓÇö coming soon"); }}
                        >
                          Import CSV
                        </button>
                      </div>
                      ...
                      {visibleCols.map((c) => (
                        <td key={`${r.id}-${String(c.key)}`} className="px-3 py-2 text-sm text-center">
                          {c.key === "organizationName"
                            ? (resolveOrgName(r.organizationId, r.organizationName, organizations) || <span className="text-secondary">ΓÇö</span>)
                            : c.render
                              ? c.render(r)
                              : ((r as any)[c.key] ?? "")
                          }
          ...
          <div className="border-b border-hairline mb-3 px-4 bg-surface">
            <div className="flex items-center gap-3 py-2">
              ...
              {d?.archived && (
                ...
                >
                  Archived{d?.archivedAt ? ` ΓÇó ${formatDate(d.archivedAt)}` : ""}
                </span>
              )}
              ...
              <div className="ml-auto flex items-center gap-2">
                ...
                <div className="text-xs text-secondary">
                  {drawerLoading ? "LoadingΓÇª" : drawerError || ""}
                </div>
              </div>
            </div>
          </div>
            ...
            <div data-1p-ignore autoComplete="off" className="space-y-4">
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                <FieldRow label="First Name">
                  {drawerEditing ? (
                    ...
                    <span className="truncate">{d?.firstName || <span className="text-secondary">ΓÇö</span>}</span>
                  )}
                </FieldRow>
                ...
                <FieldRow label="Last Name">
                  {drawerEditing ? (
                    ...
                    <span className="truncate">{d?.lastName || <span className="text-secondary">ΓÇö</span>}</span>
                  )}
                </FieldRow>
                ...
                <FieldRow label="Nickname">
                  {drawerEditing ? (
                    ...
                    <span className="truncate">{(d as any)?.nickname || <span className="text-secondary">ΓÇö</span>}</span>
                  )}
                </FieldRow>
                ...
                <FieldRow label="Organization" className="sm:col-span-3">
                  {drawerEditing ? (
                    ...
                    >
                      <option value="">ΓÇö Select Organization ΓÇö</option>
                      {organizations.map((o) => (
                    ...
                    <span className="inline-block truncate">
                      {(() => {
                        ...
                        return name || <span className="text-secondary">ΓÇö</span>;
                      })()}
                    </span>
                  )}
                </FieldRow>
                  ...
                  <FieldRow label="Email">
                    {drawerEditing ? (
                      ...
                      d?.email ? (
                        ...
                        <span className="text-secondary">ΓÇö</span>
                      )
                    )}
                  </FieldRow>
              ...
              <SectionCard title="Sales & Marketing">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <MiniStat label="Lead Status" value={drawer.leadStatus || "ΓÇö"} />
                  ...
                  <MiniStat label="Status" value={drawer.status || "ΓÇö"} />
                </div>
              </SectionCard>

              <SectionCard title="Compliance">
                <div className="mb-2 text-xs text-secondary">System sets these from unsubscribes. Toggle ΓÇ£OverrideΓÇ¥ to set manually; action is logged on Save.</div>
                        ...
                        {Array.isArray((drawer as any)?.depositHolds) && (drawer as any).depositHolds.length
                          ? (drawer as any).depositHolds.map((h: any, i: number) => (
                            <tr key={i} className="border-t border-hairline/60">
                              <td className="px-3 py-2">{h.groupName || <span className="text-secondary">ΓÇö</span>}</td>
                              <td className="px-3 py-2">{h.queuePosition ?? <span className="text-secondary">ΓÇö</span>}</td>
                            </tr>
                          ))
          ...
          {drawerTab === "audit" && (
            <div className="space-y-4">
              ...
              <SectionCard title="Change Log">
                {auditLoading && <div className="text-secondary text-sm">Loading auditΓÇª</div>}
                      ...
                      <tbody className="divide-y divide-hairline">
                        {auditRows.map((r) => {
                          ...
                          const change = r.field
                            ? `${JSON.stringify(r.before ?? null)} ΓåÆ ${JSON.stringify(r.after ?? null)}`
                            : `${JSON.stringify(r.before ?? null)} ΓåÆ ${JSON.stringify(r.after ?? null)}`;
...
/** ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ Subcomponents ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
    ...
    <>
      ...
      <div className="fixed z-[9999] w-[320px] max-w-[calc(100vw-24px)] rounded-xl border border-hairline bg-surface text-primary shadow-[0_8px_30px_hsla(0,0%,0%,0.35)]"
        ...
        <div className="p-2 border-b border-hairline flex items-center gap-2">
          <input
            autoFocus
            placeholder={`Search ${label.toLowerCase()}ΓÇª`}
            value={q}
            onChange={(e) => setQ(getEventValue(e))}
            className="w-full rounded-md border border-hairline bg-surface px-2 py-1.5 text-sm outline-none focus:shadow-[0_0_0_2px_hsl(var(--brand-orange))]"
          />
      ...
      >
        ...
        >
          <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-3 border-b border-hairline bg-surface rounded-t-xl">
            ...
            <button onClick={onClose} className="rounded px-2 py-1 hover:bg-[hsl(var(--brand-orange))]/12" aria-label="Close">Γ£ò</button>
          </div>
